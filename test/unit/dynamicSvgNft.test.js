    ...
    : describe("Dynamic SVG NFT Unit Tests", function () {
          ...
          describe("mintNft", () => {
              it("emits an event and creates the NFT", async function () {
                  ...
                  const nftHighValue =
                      await dynamicSvgNft.getHighValueOfTokenId(0)
                  assert.equal(nftHighValue.toString(), highValue.toString())
              })
          ...
          describe("svgToImageUri", () => {
              it("Convert SVG to Uri Properly", async function () {
                  ...
                  assert.equal(lowSvgUri, lowSVGImageuri)
              })
          })
      })
